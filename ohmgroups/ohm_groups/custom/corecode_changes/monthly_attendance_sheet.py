...
def get_columns_for_days(filters: Filters) -> List[Dict]:
	...
	for day in range(1, total_days + 1):
		...
		days.append({"label": f"In Time {label}", "fieldtype": "Time", "fieldname": f"in_time_{day}", "width": 65})
		days.append({"label": f"Out Time {label}", "fieldtype": "Time", "fieldname": f"out_time_{day}", "width": 65})
...
def get_attendance_map(filters: Filters) -> Dict:
	...
	{
	        'employee1': {
	                'Morning Shift': {1: {'status': 'Present', 'in': '09:00:00', 'out': '19:00:00'}, 2: 'Absent', ...}
	...
	for att in attendance_list:
		if att.in_time:
			att.in_time = get_time(att.in_time or "")
		if att.out_time:
			att.out_time = get_time(att.out_time or "")

	for d in attendance_list:
		attendance_map.setdefault(d.employee, frappe._dict()).setdefault(d.shift, frappe._dict())
		attendance_map[d.employee][d.shift][d.day_of_month] = {'status': d.status, 'in_time': d.in_time, 'out_time': d.out_time}
...
def get_attendance_status_for_detailed_view(
	...
	for shift, status_dict in employee_attendance.items():
		...
		for day in range(1, total_days + 1):
			status = status_dict.get(day, {}).get('status')
			if status is None and holidays:
				status = get_holiday_status(day, holidays)

			abbr = status_map.get(status, "")
			row[day] = abbr
			row[f"in_time_{day}"] = status_dict.get(day, {}).get('in_time')
			row[f"out_time_{day}"] = status_dict.get(day, {}).get('out_time')
