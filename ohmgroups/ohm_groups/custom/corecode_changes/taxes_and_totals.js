	...
	calculate_item_values() {
		...
		if (!this.discount_amount_applied) {
				...
				for (const item of this.frm.doc.items || []) {
					if (item.weight_uom == "Kg"){
						frappe.model.round_floats_in(item);
						item.net_rate = item.rate;
						item.qty = item.qty === undefined ? (me.frm.doc.is_return ? -1 : 1) : item.qty;
						item.net_amount = item.amount = flt(item.rate * item.total_weight, precision("amount", item)); //core item.rate to change item.total_weight
						...
						item.net_rate = item.rate;
						item.qty = item.qty === undefined ? (me.frm.doc.is_return ? -1 : 1) : item.qty;
						item.net_amount = item.amount = flt(item.rate * item.qty, precision("amount", item)); //core item.rate to change item.total_weight
						item.item_tax_amount = 0.0;
						item.total_weight = flt(item.weight_per_unit * item.stock_qty);
