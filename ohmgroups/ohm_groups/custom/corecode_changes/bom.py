					...
					frappe._dict(
						{
							...
							"sourced_by_supplier": d.sourced_by_supplier,
							"job_":d.job_ #  core code - new column added job_
						}
					)
				)
...
def get_bom_items_as_dict(
	...
	query = """select
			...
			where
				...
				order by idx"""
# core code - new column added bom_item.job_ -> 1038
	...
	if cint(fetch_exploded):
		query = query.format(
			...
			qty_field="stock_qty",
			select_columns=""",bom_item.job_, bom_item.source_warehouse, bom_item.operation,
...
# core code - new column added bom_item.job_ -> 1064
		...
		query = query.format(
			...
			select_columns=""", bom_item.job_,bom_item.uom, bom_item.conversion_factor, bom_item.source_warehouse,
