...
class WorkOrder(Document):
	...
	def prepare_data_for_job_card(self, row, index, plan_days, enable_capacity_planning):
		...
		for i in self.required_items:
			v = i.job_
			if v == 1:
				items = job_card_doc.items
				job_card_doc.update({"items":items[0:1]})
				job_card_doc.save()
				jc= frappe.as_json(job_card_doc)
				jc = json.loads(jc)
				for i in items[1:]:
					i= frappe.as_json(i)
					i = json.loads(i)
					...
					del i['parent']
					del i['idx']
					jobc_ = frappe.new_doc("Job Card")
					jobc_.update(jc)
					jobc_.update({"items":[i]})
					jobc_.insert()
