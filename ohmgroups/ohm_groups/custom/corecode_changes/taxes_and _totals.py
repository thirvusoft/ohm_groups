		...
		if not self.discount_amount_applied:
			for item in self.doc.get("items"):
				...
				if item.weight_uom == "Kg":
					...
					if not item.total_weight and self.doc.get("is_return"):
						item.amount = flt(-1 * item.rate, item.precision("amount"))
					elif not item.total_weight and self.doc.get("is_debit_note"):
						item.amount = flt(item.rate, item.precision("amount"))
					else:
						item.amount = flt(item.rate * item.total_weight, item.precision("amount"))
				else:
