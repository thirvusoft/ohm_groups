...
def qc_report(item_code, name):
    ...
    v= item_code.split('-')
    if v[-1] != "FG":
        fg_item = frappe.get_doc("Item",{"name":item_code})
        if fg_item.has_variants == 1:
            item = frappe.get_all("Item Variant Attribute",filters={"variant_of":["in",item_code], "attribute_value":["=","Fg"]},fields=["parent","attribute_value"])
            for i in item:
                items =i.parent
            # items.append({"item":i.parent})
                    # item = i.parent
